...
MiniJump.jump = function(target, backward, till, n_times)
  ...
  local function highlight_eyeliner(options)
    local success, eyeliner = pcall(require, 'eyeliner')
    if success and eyeliner and eyeliner.highlight then
      eyeliner.highlight(options or {})
    end
  end
  ...
  H.map('n', 'L', function()
    BrandNewJump = true
    highlight_eyeliner { forward = true }
    return '<Cmd>lua MiniJump.smart_jump(false, false)<CR>'
  end, { expr = true, desc = 'Jump forward with Eyeliner' })

  H.map('n', 'H', function()
    BrandNewJump = true
    highlight_eyeliner { forward = false }
    return '<Cmd>lua MiniJump.smart_jump(true, false)<CR>'
  end, { expr = true, desc = 'Jump forward with Eyeliner' })
end
...
H.apply_config = function(config)
  ...
  -- Normal mode mappings
  -- H.map('n', config.mappings.forward, '<Cmd>lua MiniJump.smart_jump(false, false)<CR>', { desc = 'Jump forward' })
  H.map('n', config.mappings.forward, function()
    highlight_eyeliner { forward = true }
    return '<Cmd>lua MiniJump.smart_jump(false, false)<CR>'
  end, { expr = true, desc = 'Jump forward with Eyeliner' })

  -- H.map('n', config.mappings.backward, '<Cmd>lua MiniJump.smart_jump(true, false)<CR>', { desc = 'Jump backward' })
  H.map('n', config.mappings.backward, function()
    highlight_eyeliner { backward = true }
    return '<Cmd>lua MiniJump.smart_jump(true, false)<CR>'
  end, { expr = true, desc = 'Jump backward with Eyeliner' })

  -- H.map('n', config.mappings.forward_till, '<Cmd>lua MiniJump.smart_jump(false, true)<CR>', { desc = 'Jump forward till' })
  H.map('n', config.mappings.forward_till, function()
    highlight_eyeliner { forward = true, till = true }
    return '<Cmd>lua MiniJump.smart_jump(false, true)<CR>'
  end, { expr = true, desc = 'Jump forward till with Eyeliner' })

  -- H.map('n', config.mappings.backward_till, '<Cmd>lua MiniJump.smart_jump(true, true)<CR>', { desc = 'Jump backward till' })
  H.map('n', config.mappings.backward_till, function()
    highlight_eyeliner { backward = true, till = true }
    return '<Cmd>lua MiniJump.smart_jump(true, true)<CR>'
  end, { expr = true, desc = 'Jump backward till with Eyeliner' })

  -- H.map('n', config.mappings.repeat_jump, '<Cmd>lua MiniJump.jump()<CR>', { desc = 'Repeat jump' })
  H.map('n', config.mappings.repeat_jump, function()
    highlight_eyeliner()
    return '<Cmd>lua MiniJump.jump()<CR>'
  end, { expr = true, desc = 'Repeat jump with Eyeliner' })
  ...
  H.map('x', config.mappings.forward, function()
    highlight_eyeliner { forward = true }
    return '<Cmd>lua MiniJump.smart_jump(false, false)<CR>'
  end, { expr = true, desc = 'Jump forward with Eyeliner' })

  H.map('x', config.mappings.backward, function()
    highlight_eyeliner { backward = true }
    return '<Cmd>lua MiniJump.smart_jump(true, false)<CR>'
  end, { expr = true, desc = 'Jump backward with Eyeliner' })

  H.map('x', config.mappings.forward_till, function()
    highlight_eyeliner { forward = true, till = true }
    return '<Cmd>lua MiniJump.smart_jump(false, true)<CR>'
  end, { expr = true, desc = 'Jump forward till with Eyeliner' })
  ...
  local fwd_expr = H.make_expr_jump(false, false)
  local bwd_expr = H.make_expr_jump(true, false)
  local fwd_till_expr = H.make_expr_jump(false, true)
  local bwd_till_expr = H.make_expr_jump(true, true)
  local rep_expr = H.make_expr_jump()

  H.map('o', config.mappings.forward, function()
    highlight_eyeliner { forward = true }
    return fwd_expr()
  end, { expr = true, desc = 'Jump forward' })

  H.map('o', config.mappings.backward, function()
    highlight_eyeliner { backward = true }
    return bwd_expr()
  end, { expr = true, desc = 'Jump backward' })

  H.map('o', config.mappings.forward_till, function()
    highlight_eyeliner { forward = true, till = true }
    return fwd_till_expr()
  end, { expr = true, desc = 'Jump forward till' })

  H.map('o', config.mappings.backward_till, function()
    highlight_eyeliner { backward = true, till = true }
    return bwd_till_expr()
  end, { expr = true, desc = 'Jump backward till' })

  H.map('o', config.mappings.repeat_jump, function()
    highlight_eyeliner()
    return rep_expr()
  end, { expr = true, desc = 'Repeat jump' })
end
...
H.make_expr_jump = function(backward, till)
  return function()
    ...
    vim.schedule(function()
      if H.cache.has_changed_cursor then
        return
      end
      vim.cmd 'undo!'
    end)
